...
# $WATCHDOG_ESCALATE_TO with concrete evidence. A JSON ledger de-dups: the same
# incident only re-alerts every $WATCHDOG_REALERT_TICKS ticks or when its symptom
# changes, so a persistent stall does not spam every tick.
#
# Config (all env-overridable):
#   WATCHDOG_ESCALATE_TO   recipient for incident mail (REQUIRED — no default;
#                          set in your pack config, e.g. to your overseer role)
...
[ -f "$__SCRIPT_DIR/_bd_trace.sh" ] && . "$__SCRIPT_DIR/_bd_trace.sh" "rig-liveness-watchdog"

if [ -z "${WATCHDOG_ESCALATE_TO:-}" ]; then
    echo "rig-liveness-watchdog: WATCHDOG_ESCALATE_TO must be set in your pack config (e.g. to your overseer role)" >&2
    exit 1
fi
