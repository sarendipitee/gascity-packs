...
#   WATCHDOG_ESCALATE_TO   recipient for incident mail        (REQUIRED — no default; script exits 1 if unset)
#   WATCHDOG_REFINERY_ROLE role suffix to match as refinery   (default: refinery)
#   WATCHDOG_WITNESS_ROLE  role suffix to match as witness    (default: witness)
...
REFINERY_ROLE="${WATCHDOG_REFINERY_ROLE:-refinery}"
WITNESS_ROLE="${WATCHDOG_WITNESS_ROLE:-witness}"

if [ -z "${WATCHDOG_ESCALATE_TO:-}" ]; then
    echo "rig-liveness-watchdog: WATCHDOG_ESCALATE_TO is required but not set" >&2
    exit 1
fi
ESCALATE_TO="$WATCHDOG_ESCALATE_TO"
...
witness_state=$(role_state "$WITNESS_ROLE")
refinery_state=$(role_state "$REFINERY_ROLE")
...
Action: gc session reset $RIG/<witness-alias>  (or respawn the rig witness)."
    ...
    hb=$(role_heartbeat "$WITNESS_ROLE")
...
# Note: gc bd list --rig requires gc >= the version that added per-rig bd scoping
# (first appeared after the rig-scoped exec-order fan-out landed).
queue=$(gc bd list --rig "$RIG" --json --limit=0 2>/dev/null \
    | jq --arg role "$REFINERY_ROLE" '[ .[]
            | select((.assignee // "") | test($role))
